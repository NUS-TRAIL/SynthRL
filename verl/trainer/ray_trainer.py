...
class RayPPOTrainer:
    ...
    def _save_checkpoint(self) -> None:
        ...
        # Read the current previous global step (if any)
        ...
        # Update the checkpoint tracker with current global step
        with open(last_global_step_path, "w") as f:
            f.write(str(self.global_step))
        
        # Now, convert the previous checkpoint to HF format if it exists
        if previous_step is not None:
            previous_folder_path = os.path.join(self.config.trainer.save_checkpoint_path, f"global_step_{previous_step}")
            previous_actor_path = os.path.join(previous_folder_path, "actor")
            
            if os.path.exists(previous_actor_path):
                try:
                    import subprocess
                    
                    # Use current working directory as the BASE_DIR
                    base_dir = os.getcwd()
                    
                    env = os.environ.copy()
                    env['BASE_DIR'] = base_dir
                    env['MODEL_PATH'] = previous_actor_path

                    print(f"Converting previous checkpoint at global_step_{previous_step}...")
                    ...
                    result = subprocess.run([
                        ...
                        if find "$MODEL_PATH/huggingface" -maxdepth 1 -name "*.safetensors" -print -quit 2>/dev/null; then
                            echo "Detected .safetensors file in the huggingface directory."
                            echo "Deleting all .pt files in the actor directory..."
                            find "$MODEL_PATH" -maxdepth 1 -name "*.pt" -type f -delete
                            echo "Deletion completed."
                        else
                            echo "No .safetensors file detected in the huggingface directory. Skipping .pt file deletion."
                        fi
                        '''
                    ], env=env, check=False, text=True, capture_output=True)
                    
                    print("Previous model conversion script output:")
                    print(result.stdout)
                    
                    if result.returncode != 0:
                        print(f"Error: Conversion script exited with code {result.returncode}")
                        print(f"Error output: {result.stderr}")
                
                except Exception as e:
                    print(f"Error during previous model conversion: {str(e)}")
            
            # Finally, handle obsolete checkpoints
            remove_obsolete_ckpt(
                self.config.trainer.save_checkpoint_path, self.global_step, self.config.trainer.save_limit
            )
            
            print(f"Saved checkpoint at global_step_{self.global_step} (not converting to HF format)")
